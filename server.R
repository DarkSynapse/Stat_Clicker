...
sw_plot <- function(my_dt, 
  ...
  ggplot( my_dt, aes(x = as.factor(ID), OST)) +
    geom_bar(stat="identity", width=0.7, alpha= 1, colour = '#0073C2FF',fill = '#0073C2FF') +
    geom_bar(aes(y = OST2), alpha= 1, stat = "identity", position = "dodge", width = 0.7,  colour = '#CD534CFF',fill = '#CD534CFF' ) +
    geom_point(data=my_dt %>% filter(OSS == 1), aes(x=ID,  y=OST + 0.8), pch = 4, size = 2) +
    geom_segment(data=my_dt %>% filter(OSS == 0), aes(x=ID, xend=ID, y=OST + 0.7, yend=OST + 1.3), size = 1, arrow=arrow(type="closed", length=unit(0.05,"in"))) +
...
shinyServer(function(input,output,session){
  ...
  df1 = data.frame(Main_time=Main_time, 
                   Add_time=Add_time, 
                   Fact_time=Fact_time,
                   Fact_stat=Fact_stat,
                   Death_1_0=Death_1_0
  ...
  observeEvent(
    input$table2$changes$changes, # observe if any changes to the cells of the rhandontable
    {
  ...
  output$menu <- renderMenu({
    sidebarMenu(
      menuItem("Swimmer plot", icon = icon("line-chart"), tabName = "dashboard"),
      menuItem("Waterfall plot", icon = icon("line-chart"), tabName = "wf_tab"),
  ...
  output$wf_plot <- renderPlot(
    width = function() input$wf_width,
    height = function() input$wf_height,
    res = 96,
    
    {
      wf_plot(my_dt = datavalues_wf$data, 
